...
static const coop_cfg_key coop_server_config_keys[] = {
	...
	{"coop_player_community_off", coop_cfg_switch, coop_cfg_control},   // off (the community assignment is ON) | server | Control arm: co-op player bodies keep the plain "actor" community cloned from entity 0 (neutral to every faction) instead of actor_stalker.
	...
	{"coop_sound_forward_off", coop_cfg_switch, coop_cfg_control},   // off (forwarding is ON) | server, client | Control arm: clients do not forward their actor's AI sound events and the server drops any it receives (monsters cannot hear players, as before §3.4 hearing fix (B)).
	...
	{"coop_reach_wait", coop_cfg_switch, coop_cfg_control},   // off on an engine without level.coop_reach_wait_default (<= 07e5a9ba), where it opts in | server | Gamedata mp_coop_reach_wait: an online squad near a player whose target is on another map waits via GAMMA's fake_target instead of being held idle by the engine.
	{"coop_reach_wait_noleash", coop_cfg_switch, coop_cfg_control},   // off (the leash is ON) | server | Control arm: a waiting squad random-walks cover to cover instead of being walked back beyond 20 m of where it started waiting.
	{"coop_reach_wait_off", coop_cfg_switch, coop_cfg_control},   // off (reach-wait is ON on engines exporting level.coop_reach_wait_default) | server | Control arm and Caden's one switch: mp_coop_reach_wait stays out; the engine hold still catches off-level destinations.
	...
	{"coop_revive_off", coop_cfg_switch, coop_cfg_control},   // off (revive reset is ON) | server | Control arm: §9 (iii) co-op revive does not undo the death state.
