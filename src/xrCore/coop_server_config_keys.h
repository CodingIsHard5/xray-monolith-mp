...
static const coop_cfg_key coop_server_config_keys[] = {
	...
	{"coop_player_community", coop_cfg_switch, coop_cfg_tunable},   // off (§3.4 scope 2, OFF until C1-C3 pass) | server | Co-op player bodies carry the actor_stalker community instead of the plain "actor" cloned from entity 0, so hostile factions treat players as they treat the single-player actor.
