	...
	case GE_GAME_EVENT:
		{
			...
			// MP fork, 2026-09-25 — the GE_GAME_EVENT gate in LOG-ONLY form (Overseer ruling): every remote game-event
			// type is logged WITH ITS SENDER, and nothing is denied. The census so far is 7 headless runs (the only type
			// a normal client sent was GAME_EVENT_COOP_RESPAWN = 47); Caden's two-client session will exercise types a
			// headless client never sends, and enforcement is drawn from that census in the build after. First line per
			// (sender, type), then every 100th.
			if (m_enet && m_enet->running() && m_enet->owns(sender.value()))
			{
				static xr_map<u64, u32> s_ge;
				u64 const key = (u64(sender.value()) << 16) | u64(game_event_type);
				u32 const c = ++s_ge[key];
				if (c == 1 || (c % 100) == 0)
					Msg("[GEVENT] remote client 0x%08x sent game event type %u (count %u for this sender) — logged, not gated",
					    sender.value(), (u32)game_event_type, c);
			}
